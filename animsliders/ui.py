...
class AS_PT_sliders(Panel):
    ...
    def step_button(self, row, slot, factor, icon='',
    ...
    def slider_box(self, layout, slot, index=0, is_collection=True):
        ...
        if slot.modal_switch == False:
            ...
            if slot.overshoot == True:
                for f in [-2, -1.5]:
                    self.step_button(row, slot, factor=f, text=' ', icon='', is_collection=is_collection)

            self.step_button(row, slot, factor=-1, text='',
                             icon='CHECKBOX_DEHLT', emboss=False, active=True, is_collection=is_collection)

            for f in [-0.75, -0.5, -0.25]:
                self.step_button(row, slot, factor=f, text=' ', icon='', is_collection=is_collection)

            self.step_button(row, slot, factor=0, text='', icon='ANTIALIASED',
    ...
    def draw(self, context):
        ...
        if key_utils.selected_keys_global == {}:
            key_utils.get_selected_global()

        # if key_utils.ref_frames_global == {}:
        ...
        self.slider_box(layout, item, is_collection=False)
        ...
        for slot in slots:
            ...
            # if key_utils.ref_frames_global == {}:
            #     slot.left_ref_frame = 0
            #     slot.right_ref_frame = 0

            self.slider_box(layout, slot, index)
...
class AS_PT_clone(Panel):
    ...
    def draw(self, context):
        clones = context.scene.animsliders.clone_data
        ...
        row.prop(clones, 'cycle_before', text='')
        row = layout.row()
        # row.alignment = 'RIGHT'
        row.label(text='Cycle After')
        row.prop(clones, 'cycle_after', text='')
